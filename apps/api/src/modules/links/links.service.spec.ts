...
describe('LinksService', () => {
  ...
  it('throws NotFoundException when the user has no profile', async () => {
    ...
    await expect(linksService.list('user-without-profile')).rejects.toThrow(NotFoundException);
  });
  ...
  it('creates a link with an auto-incremented order when none is given', async () => {
    ...
    prismaMock.link.create.mockImplementation(({ data }) => Promise.resolve(data));

    const created = await linksService.create('user-1', { title: 'Nuevo enlace' } as any);
  ...
  it('rejects updating a link that belongs to another profile', async () => {
    prismaMock.profile.findUnique.mockResolvedValue({ id: 'profile-1' });
    prismaMock.link.findUnique.mockResolvedValue({ id: 'link-1', profileId: 'other-profile' });
